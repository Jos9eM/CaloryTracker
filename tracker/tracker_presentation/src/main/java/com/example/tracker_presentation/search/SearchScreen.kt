...
fun SearchScreen(
    ...
    Column(
        ...
        SearchTextField(text = state.query, onValueChange = {
            viewModel.onEvent(SearchEvent.OnQuerySearch(it))
        }, shouldShowHint = state.isHintVisible, onSearch = {
            keyboardController?.hide()
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            items(state.trackableFood) { food ->
                TrackableFoodItem(trackableFoodUiState = food, onClick = {
                    viewModel.onEvent(SearchEvent.OnToggleTrackableFood(food = food.food))
                }, onAmountChange = {
                    viewModel.onEvent(
                        SearchEvent.OnAmountForFoodChange(
                            food = food.food, amount = it
                        )
                    )
                }, onTrack = {
                    keyboardController?.hide()
