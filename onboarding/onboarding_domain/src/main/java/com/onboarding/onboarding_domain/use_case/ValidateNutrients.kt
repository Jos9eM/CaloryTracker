package com.onboarding.onboarding_domain.use_case

import com.calorieTracker.core.util.UiText
import com.onboarding.onboarding_domain.R

class ValidateNutrients {

    operator fun invoke(
        carbsRatioText: String, proteinRatioText: String, fatRatioText: String
    ): Result {
        ...
        return Result.Success(carbsRatio / 100f, proteinsRatio / 100f, fatRatio / 100f)
    }

    sealed class Result {
        data class Success(
            val carbsRatio: Float, val proteinRatio: Float, val fatRatio: Float
        ) : Result()
